...
def crawl_boss_zhipin(city_name='北京', city_code=None, keyword='游戏测试', total_pages=5,
    ...
    filters: 采集时生效的单选筛选条件 {字段: 选中值}（不满足的岗位不写入CSV）
    multi_selected: 采集时生效的多选筛选条件 {字段: 勾选值集合}（与 filters 配套）
    返回最终保存的CSV文件路径
    """
    def say(msg):
        if on_log:
    ...
    with f:
        ...
        say('正在连接调试模式 Chrome（127.0.0.1:9222）...')
        co = ChromiumOptions()
        ...
        for page in range(1, total_pages + 1):
            ...
            try:
                ...
                for job in job_list:
                    ...
                    if on_job:
                        on_job(job_info)
                if filtered_out or page_new < len(job_list):
                    parts = []
                    if filtered_out:
                        parts.append(f'{filtered_out} 条不符合筛选')
                    dup_skipped = len(job_list) - page_new - filtered_out
                    if dup_skipped > 0:
                        parts.append(f'{dup_skipped} 条重复')
                    say(f'第{page}页：接口返回 {len(job_list)} 条，其中 {"、".join(parts)} 已跳过，实际写入 {page_new} 条')
                else:
...
class MultiSelectDropdown(ctk.CTkButton):
    ...
    def _on_any_click(self, event):
        ...
        # 点在下拉面板内、或点在自己按钮上（开/关由 _toggle 处理）：不在这里收起
        if w is not None and (str(w).startswith(str(self._popup)) or str(w).startswith(str(self))):
            return
        self._close_popup()
...
class BossGuiApp(ctk.CTk):
    ...
    def _schedule_keyword_refresh(self):
        ...
        if hasattr(self, '_kw_timer') and self._kw_timer:
            try:
                ...
                pass
        self._kw_timer = self.after(400, self._refresh_keyword_presets)
    ...
    def _start_crawl(self):
        ...
        # 只清空上次的采集数据，保留用户已配置的筛选条件（避免白配）
        self._clear_jobs(silent=True, reset_filters=False)
        self._log_ui(f'▶ 开始采集：{city} · {keyword}，共 {pages} 页（后台运行中，界面可正常操作）')

        # 快照当前筛选条件：本次采集写CSV时也按此过滤（不满足条件的岗位不写入）
        ...
        multi_selected = {f: set(self.multi_selected[f]) for f in MULTI_SELECT_FIELDS}

        worker = threading.Thread(
            target=self._worker, args=(city, city_code, keyword, pages, filters, multi_selected), daemon=True)
        worker.start()

    def _worker(self, city, city_code, keyword, pages, filters=None, multi_selected=None):
        try:
            crawl_boss_zhipin(
                ...
                filters=filters, multi_selected=multi_selected)
        except Exception as e:
    ...
    def _rebuild_list(self):
        ...
        # 修复：重建列表后强制刷新滚动区域并回到顶部，避免 CTkScrollableFrame 的
        # scrollregion 停留在旧数据高度，导致新按钮渲染到可视区外（列表看似空白）
        try:
            self.job_list_box._parent_canvas.configure(scrollregion=self.job_list_box._parent_canvas.bbox('all'))
            self.job_list_box._parent_canvas.yview_moveto(0)
        except Exception:
            pass
    ...
    def _export_all(self):
        # 导出当前列表可见的岗位（即筛选后的结果）
        self._export_jobs(self.filtered_jobs, '当前列表的全部岗位')
